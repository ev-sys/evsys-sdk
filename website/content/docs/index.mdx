---
title: Introduction
description: A declarative framework for LLM training experiments — SFT, RL, and distillation from a single YAML, built so coding agents can launch experiments, learn from them, and train models continuously.
---
...
<Callout title="Continually learning — built in">
The mission isn't just a slogan. The `continual` modifier trains a model across
an ordered list of datasets, each stage resuming from the **previous stage's
weights** — so the model accumulates skills instead of restarting from base.
See [Continual learning](/docs/concepts/experiments#continual-learning).
</Callout>
...
#### ⚙️ The SDK handles everything else
...
<Cards>
  ...
  <Card title="🔁 Continual learning" href="/docs/concepts/experiments#continual-learning">
    Chain a model across datasets — each stage builds on the last stage's weights.
  </Card>
</Cards>
