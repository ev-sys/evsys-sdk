...
# cells: (seq, batch, rank, adapters). Axes: seq ladder, batch sweep, rank, multiLoRA count.
CELLS = json.loads(os.environ.get("CELLS", json.dumps(
    [[2048, 1, 16, 1], [8192, 1, 16, 1], [16384, 1, 16, 1], [32768, 1, 16, 1],
     [2048, 2, 16, 1], [2048, 4, 16, 1], [2048, 8, 16, 1], [8192, 2, 16, 1], [8192, 4, 16, 1],
     [2048, 1, 64, 1], [2048, 1, 16, 2], [2048, 1, 16, 4], [2048, 1, 16, 8], [8192, 1, 16, 4]])))
...
def main():
    ...
    # Build the PEFT model ONCE with every adapter any cell needs (rank-16 pool
    # a0..aMAX for multiLoRA, plus one rank-64 adapter r64), then just select.
    max_ad = max(a for _, _, _, a in CELLS)
    ranks = sorted({r for _, _, r, _ in CELLS})
    lora16 = LoraConfig(r=16, lora_alpha=32, target_modules=["q_proj", "k_proj", "v_proj", "o_proj"],
                        lora_dropout=0.0, task_type="CAUSAL_LM")
    peft = get_peft_model(model, lora16, adapter_name="a0")
    for i in range(1, max_ad):
        peft.add_adapter(f"a{i}", lora16)
    for rk in ranks:
        if rk != 16:
            peft.add_adapter(f"r{rk}", LoraConfig(r=rk, lora_alpha=rk * 2,
                target_modules=["q_proj", "k_proj", "v_proj", "o_proj"], lora_dropout=0.0, task_type="CAUSAL_LM"))
    peft.train(); peft.gradient_checkpointing_enable()
    ...
    results = []
    for seq, batch, rank, adapters in CELLS:
        try:
            import torch
            names = [f"a{i}" for i in range(adapters)] if rank == 16 else [f"r{rank}"]
            ...
            def one():
                tot = 0.0
                for nm in names:                       # each adapter its own fwd/bwd (no batched multiLoRA kernel)
                    if len(names) > 1: peft.set_adapter(nm)
            ...
            for _ in range(STEPS):
                last = one(); done += 1
                if time.time() - t > CELL_BUDGET:
                    break
            torch.cuda.synchronize(); dt = time.time() - t
            toks = seq * batch * adapters * done / dt   # adapters process independent token streams
            peak = sum(torch.cuda.max_memory_allocated(i) for i in range(torch.cuda.device_count())) / 1e9
            r = dict(card=CARD, model=MODEL, seq=seq, batch=batch, rank=rank, adapters=adapters,
            ...
            results.append(r); say(f"cell s{seq}b{batch}r{rank}a{adapters}", json.dumps(r))
        except torch.cuda.OutOfMemoryError:
            say(f"OOM s{seq}b{batch}r{rank}a{adapters}", json.dumps(dict(card=CARD, seq=seq, batch=batch, rank=rank, adapters=adapters, limit="OOM")))
            torch.cuda.empty_cache()
        except Exception as e:
            say(f"err s{seq}b{batch}a{adapters}", f"{type(e).__name__}: {e}\n{traceback.format_exc()[:1200]}")
            torch.cuda.empty_cache()
    samp.on = False
