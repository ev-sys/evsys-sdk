...
say "N:$EVSYS_JOB_ID" "agent up $(hostname); resume_step=$EVSYS_RESUME_STEP; vol=$EVSYS_VOLUME"
( while true; do sleep 150; say "hb:$EVSYS_JOB_ID" "$(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | tr '\n' ';') run:$(tail -c 200 /root/evsys_run.log 2>/dev/null | tr '\n' ' ') srv:$(tail -c 120 /root/srv.log 2>/dev/null | tr '\n' ' ')"; done ) &
...
setsid nohup uv run --extra tinker --extra megatron python -m skyrl.tinker.api \
  --host 0.0.0.0 --port 8000 --base-model Qwen/Qwen3-4B-Instruct-2507 \
  --backend megatron --backend-config '{
    ...
    "trainer.policy.model.lora.max_loras": 8,
    "trainer.policy.model.lora.max_cpu_loras": 8,
    "trainer.logprobs_chunk_size": 1024,
    "trainer.fused_lm_head_logprob": true
  }' > /root/srv.log 2>&1 < /dev/null &
OK=""
for i in $(seq 1 150); do curl -sf --max-time 3 http://127.0.0.1:8000/api/v1/healthz >/dev/null && { OK=1; break; }; sleep 8; done
[ -n "$OK" ] || { say "FAIL:$EVSYS_JOB_ID" "server never healthy"; sayfile "SRVLOG:$EVSYS_JOB_ID" /root/srv.log; exit 1; }
say "N:$EVSYS_JOB_ID" "skyrl server healthy"
