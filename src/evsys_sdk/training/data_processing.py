r"""Turn :class:`~evsys_sdk.training.rollout.TrajectoryGroup`\s into
...
from .rollout import TrajectoryGroup, Turn
...
def assemble_training_data(
    ...
    **One Datum per assistant turn** — a single-turn trajectory yields one
    Datum; a multi-turn trajectory yields one per turn, each carrying the same
    trajectory-level advantage. Each Datum carries:
      ...
      * ``loss_fn_inputs["logprobs"]``: sampler's per-position logprobs for the
        completion tokens (zero on prompt positions). Used by the IS loss.
      * ``loss_fn_inputs["advantages"]``: the trajectory's group-normalized
        reward, broadcast to every completion position; 0 on prompt positions.
