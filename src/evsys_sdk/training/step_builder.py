"""SDFT / RL step builders + shared datum-metric helpers.

A StepBuilder turns a step index into a
:class:`~evsys_sdk.training.loop.TrainingBatch` and computes per-step
algorithm-specific metrics; the loop drives everything else. SFT's batch
logic now lives directly on
:class:`~evsys_sdk.algorithms.sft.SFT` (the algorithm IS its own
step builder via :class:`~evsys_sdk.algorithms.base.BaseAlgorithm`); the SDFT
and RL builders here are mid-migration to the same shape.
"""
...
__all__ = ["RLDataset", "RLStepBuilder", "SDFTDataset", "SDFTStepBuilder"]
