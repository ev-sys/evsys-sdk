...
Shared by both ComposioSearchEvalResult and ModelEvalResult — they have the
same per-row, per-query shape so the same scorer works on both. The caller
just supplies a function that maps one query-result to a bool ``found``.
"""
...
def composio_query_found(qresult: dict[str, Any], expected_slug: str, matcher: AliasMatcher) -> bool:
    """Composio search hit if any accepted slug appears in primary OR related."""
    if qresult.get("error"):
        return False
    returned = list(qresult.get("primary", [])) + list(qresult.get("related", []))
    return matcher.found_in(expected_slug, returned)


def composio_query_found_primary(qresult: dict[str, Any], expected_slug: str, matcher: AliasMatcher) -> bool:
    """Strict variant: only count primary hits."""
    if qresult.get("error"):
        return False
    return matcher.found_in(expected_slug, list(qresult.get("primary", [])))
