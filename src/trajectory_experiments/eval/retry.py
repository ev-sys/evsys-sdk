...
# transient HTTP error. We import lazily so the SDK doesn't hard-depend on
# httpx or composio_client.
def _default_retriable_exceptions() -> tuple[type[BaseException], ...]:
    ...
    try:
        ...
        pass
    try:
        from composio_client import APIConnectionError, APITimeoutError  # type: ignore
...
class RetryFailure:
    ...
    context: str
    """Free-form label for the call site (e.g. 'composio_search:row42:query1')."""
    exception_type: str
