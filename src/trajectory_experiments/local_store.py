...
(default ``./trajectory_experiments``). This guarantees no data is lost even
...
class LocalExperimentStore:
    ...
    def create_generation(self, generation_id: str, payload: dict[str, Any]) -> None:
        with self._lock:
            row = {"id": generation_id, "_created_at": time.time(), **payload}
            self._write_json(self._gen_dir(generation_id) / LOCAL_GENERATION_FILE, row)
        log.debug("local: wrote generation %s", generation_id)

    def update_generation(self, generation_id: str, patch: dict[str, Any]) -> None:
        with self._lock:
            self._merge_json(self._gen_dir(generation_id) / LOCAL_GENERATION_FILE, patch)
