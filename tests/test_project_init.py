...
def test_readme_references_layout(tmp_path: Path):
    ...
    for tok in ("data/", "src/", "experiments/", "evsys new-experiment",
...
def test_src_init_imports_extension_modules(tmp_path: Path):
    init_project(tmp_path / "demo")
    init = (tmp_path / "demo" / "src" / "__init__.py").read_text()
    assert "from . import verifiers" in init
    assert "from . import metrics" in init
    assert "from . import transforms" in init


def test_src_package_is_importable(tmp_path: Path, monkeypatch):
    """Importing the scaffolded src package must not raise."""
    path = init_project(tmp_path / "demo")
...
def test_init_into_empty_existing_dir(tmp_path: Path):
    ...
    target.mkdir()
    init_project(target)
    assert (target / "src" / "__init__.py").is_file()
...
def test_cli_init_project_happy(tmp_path: Path, capsys):
    ...
    assert (tmp_path / "via_cli" / "src" / "__init__.py").is_file()
