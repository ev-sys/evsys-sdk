...
def _corpus(root):
    (root / "alice@corp.com").mkdir(parents=True)
    (root / "alice@corp.com" / "m1.txt").write_text("The export button is broken on billing.")
    (root / "bob@corp.com").mkdir(parents=True)
    (root / "bob@corp.com" / "note.txt").write_text("Prefers concise answers.")


def _spec(path, state_dir, **extra):
    return SystemConfig(context={"context_sources": [
        {"kind": "directory", "params": {"path": str(path), "kind": "email", **extra},
         "state_dir": str(state_dir)}]}).context.context_sources


def test_context_item_roundtrip():
    it = ContextItem("i1", "directory", "email", "hi", entity="alice", metadata={"k": "v"})
    d = it.to_dict()
    assert d == {"item_id": "i1", "source": "directory", "kind": "email",
                 "content": "hi", "entity": "alice", "metadata": {"k": "v"}}


def test_directory_adapter_maps_files(tmp_path):
    _corpus(tmp_path / "mail")
    (_, src), = build_context_sources(_spec(tmp_path / "mail", tmp_path / ".evsys"))
    src.run_once()
    ...
    assert set(by_entity) == {"alice@corp.com", "bob@corp.com"}      # folder → entity
    assert by_entity["alice@corp.com"]["kind"] == "email"
    assert "export button" in by_entity["alice@corp.com"]["content"]  # body → content


def test_store_and_cursor_dedupe(tmp_path):
    _corpus(tmp_path / "mail")
    specs = _spec(tmp_path / "mail", tmp_path / ".evsys")
    assert run_context_pull(specs) == 2           # first pull lands both
    assert run_context_pull(specs) == 0           # nothing new → cursor dedupes
    (tmp_path / "mail" / "carol@corp.com").mkdir()
    (tmp_path / "mail" / "carol@corp.com" / "x.txt").write_text("new ticket")
...
def test_incremental_since_skips_old(tmp_path):
    ...
    (_, src), = build_context_sources(_spec(tmp_path / "mail", tmp_path / ".evsys"), store=store)
    ...
    try:
        ...
        cfg = SystemConfig(
            ...
            context={"context_sources": [{"kind": "directory",
                                          "params": {"path": str(tmp_path / "mail"), "kind": "email"},
...
def test_live_directory_env_gated(tmp_path):
    ...
    _corpus(tmp_path / "mail")
    assert run_context_pull(_spec(tmp_path / "mail", tmp_path / ".evsys")) == 2
