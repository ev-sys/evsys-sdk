...
from evsys_sdk.compute.provisioner_nebius import (DATA_DEVICE_ID,
                                                  NebiusProvisioner)
...
def test_alive_states(prov):
    ...
    api.fail_get = IOError("404 instance not found")
    assert p.alive(h) is False
    api.fail_get = IOError("timeout")           # transient -> assume alive
    assert p.alive(h) is True
