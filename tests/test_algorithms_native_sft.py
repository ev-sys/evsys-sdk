"""End-to-end test of ``evsys_sdk.algorithms.native_sft.NativeSFT``.
...
import evsys_sdk.algorithms.native_sft as native_sft_module
from evsys_sdk.algorithms.native_sft import NativeSFT, NativeSFTConfig
...
def patched_tinker_backend(monkeypatch):
    ...
    This lets us exercise NativeSFT.train end-to-end (the composer plus the
    ...
    monkeypatch.setattr(native_sft_module.TinkerBackend, "create", _factory)
    return mock


@pytest.fixture
def ctx(tmp_path: Path):
    """Minimal RunContext satisfying NativeSFT's reads from `ctx.extras`."""
    rows = [
        {"messages": [
...
def test_registered_under_native_sft():
    assert get_algorithm("native_sft") is NativeSFT
...
def test_config_rejects_unknown_kwarg():
    with pytest.raises(Exception):
        NativeSFTConfig(boguous_field=True)
...
def test_train_rejects_non_tinker_backend(ctx):
    ...
    algo = NativeSFT(max_steps=2, batch_size=4)
...
def test_train_runs_end_to_end_and_returns_run_result(patched_tinker_backend, ctx):
    ...
    algo = NativeSFT(max_steps=3, batch_size=4, save_at_fractions=[1.0])
...
def test_train_logs_hyperparams_once(patched_tinker_backend, ctx):
    ...
    assert hp["algorithm"] == "native_sft"
...
def test_train_writes_one_metric_row_per_step(patched_tinker_backend, ctx):
    algo = NativeSFT(max_steps=4, batch_size=4)
    algo.train(ctx)
...
def test_save_at_fractions_resolves_to_gcd_when_reasonable():
    ...
    algo = NativeSFT(max_steps=100, save_at_fractions=[0.25, 0.5, 0.75, 1.0])
